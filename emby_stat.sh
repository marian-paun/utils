...
DB_PATH="${DB_PATH:-/volume1/home/emby/data/playback_reporting.db}"
...
command -v /opt/bin/sqlite3 >/dev/null 2>&1 || { echo "sqlite3 not found" >&2; exit 1; }

QUERY="SELECT COALESCE(SUM(PlayDuration), 0) FROM PlaybackActivity WHERE DateCreated >= datetime('now', '$TIME_WINDOW');"

TOTAL_SECONDS=$(/opt/bin/sqlite3 "$DB_PATH" "$QUERY" 2>/dev/null || echo 0)

TOTAL_MINUTES=$((TOTAL_SECONDS / 60))

OUTPUT=$(/usr/bin/printf '{"PlayMinutes":%d}\n' "$TOTAL_MINUTES")
