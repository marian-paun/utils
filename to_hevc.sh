...
# falling back to a disk-based directory otherwise.
...
# Minimum free space (in MB) required in PRIMARY_TEMP_DIR to use it.
# Adjust this value based on the typical size of your video files and your Pi's RAM.
# A value of 500-1000 MB (0.5-1 GB) is a reasonable starting point for many HD videos.
MIN_TMP_SPACE_MB=1000

# Current active temporary directory, determined at runtime
CURRENT_TEMP_DIR=""
...
# Function to get free space in MB for a given directory
get_free_space_mb() {
    local dir=$1
    # df -k outputs in 1KB blocks. We take the 4th column (Available), then divide by 1024 to get MB.
    df -k "$dir" | awk 'NR==2 {print int($4/1024)}' 2>/dev/null || echo 0
}
...
# Determine which temporary directory to use based on available space in PRIMARY_TEMP_DIR
FREE_SPACE_PRIMARY_TMP=$(get_free_space_mb "$PRIMARY_TEMP_DIR")
if (( FREE_SPACE_PRIMARY_TMP >= MIN_TMP_SPACE_MB )); then
    CURRENT_TEMP_DIR="$PRIMARY_TEMP_DIR"
    log_message "Using RAM-backed temporary directory: $CURRENT_TEMP_DIR (Available space: $(bytes_to_human_readable $((FREE_SPACE_PRIMARY_TMP * 1024 * 1024))))"
else
    CURRENT_TEMP_DIR="$FALLBACK_TEMP_DIR"
    log_message "Not enough free space in $PRIMARY_TEMP_DIR ($(bytes_to_human_readable $((FREE_SPACE_PRIMARY_TMP * 1024 * 1024))) available, requires $(bytes_to_human_readable $((MIN_TMP_SPACE_MB * 1024 * 1024)))). Using disk-based temporary directory: $CURRENT_TEMP_DIR"
fi

# Create the chosen temporary directory if it doesn't exist
mkdir -p "$CURRENT_TEMP_DIR" || { log_message "Error: Could not create temporary directory $CURRENT_TEMP_DIR. Check permissions."; exit 1; }
log_message "Temporary directory set to: $CURRENT_TEMP_DIR"
...
for ext in "${VIDEO_EXTENSIONS[@]}"; do
    ...
    find "$VIDEO_DIR" -type f -name "*.${ext}" -print0 | while IFS= read -r -d $'\0' video_file; do
        ...
        # Define temporary output filename using the chosen CURRENT_TEMP_DIR
        # Using a UUID to ensure uniqueness, preventing conflicts
        temp_output_filename=$(basename "$video_file" | sed 's/\(.*\)\..*/\1/')_$(uuidgen).mkv
        temp_output_path="${CURRENT_TEMP_DIR}/${temp_output_filename}"
...
log_message "You can safely remove the temporary directory if it's empty: $CURRENT_TEMP_DIR"
